...
//Handle profile picture update with MySQL update Query using $options array	
    if ($post['id']) {
        $sql_query = "SELECT * FROM tbl_admin_data WHERE admin_id = '" . mysqli_escape_string($conn, $post['id']) . "'";
        $resultset = mysqli_query($conn, $sql_query) or die("database error:" . mysqli_error($conn));
        if (mysqli_num_rows($resultset)) {
            $sql_update = "UPDATE tbl_admin_data set admin_image='" . mysqli_escape_string($conn, $post['image_name']) . "' WHERE admin_id = '" . mysqli_escape_string($conn, $post['id']) . "'";
