                ...
                <div class="">
                    ...
                    if ($con) {
                        $q = $con->prepare("SELECT plsmnt_id,technology_plsmnt,comp_website,num_required_students,company_name,comp_logo,placement_announcement_date"
                                ...
                                . " where status='Announced' order by placement_announcement_date");
                        $q->execute();
                        $r = $q->get_result();
                        if ($k = $r->fetch_assoc()) {
                            do {
                        ...
                        <p><div class="col-sm-2"></div><div id="' . $k['plsmnt_id'] . '" class="col-sm-4 text-danger"><b class="hover_pointer"><i class="fa fa-close"></i> Reject</b></div><div id="' . $k['plsmnt_id'] . '" class="col-sm-4 text-success"><b class="hover_pointer"><i class="fa fa-check"></i> Approve</b></div></p><br>
                        <div class="bigtext">' . $k['company_name'] . '<br><a href="' . $k['comp_website'] . '" target="blank"> ' . $k['comp_website'] . '</a></div>
                    </div></center>';
                            } while ($k = $r->fetch_array());
                        } else {
                            echo '<center><h2 class="dpink_col"><i class="fa fa-dropbox gray_col" style="font-size:45px"></i><br><br> NO PLACEMENTS ANNOUNCED.</h2></center>';
                        }
                    }
                    ?>                    
                </div>
